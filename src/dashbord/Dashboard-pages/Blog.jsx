...
import { createBlog, deleteBlog, readAllBlog, updateBlog } from "../../services/index/blog";
   ...
   const refetchSchoolBlog = async () => {
    ...
    const onSubmit = async (data) => {
        try {
          ...
          if (selectedBlogId) {
            const result = await updateBlog(token, selectedBlogId, data); // Call updateTeacher function with selectedNavbarId and data
            toast.success('Blog updated successfully!');
          } else {
            // Create new teacher
            const result = await createBlog({ token, formData: data });
      ...
      const handleUpdate = async (iconId) => {
        try {
          ...
          const token = "your_token_here"; // Replace with actual token retrieval logic
          const teacherInfo = await readAllBlog(token, iconId); // Implement a function to fetch teacher information
          reset(teacherInfo); // Assuming reset is a function to reset the form with new values
          setSelectedBlogId(iconId); // Set the selectedNavbarId when update button is clicked
        } catch (error) {
          console.error(error.message);
    ...
    const handleRemove = async (iconId) => {
        try {
          const token = "your_token_here"; // Replace with actual token retrieval logic
          await deleteBlog(token, iconId); // Call deleteTeacher function with iconId
    ...
    return (
        <div>
           <h2 className="text-gray-900 border-l-4 border-[#1eb2a6] pl-2 lg:text-2xl md:text-xl text-lg font-semibold capitalizemy-4"> Blog</h2>
             <h2 className="text-gray-900  mx-auto md:w-[30%] text-center w-[50%] my-6 border-x-2 border-[#1eb2a6] px-2 lg:text-2xl md:text-xl text-lg font-semibold "> Blog Form</h2> 

            <form  onSubmit={handleSubmit(onSubmit)} className="bg-white shadow-md md:mb-20 mb-10 md:px-3 px-1 md:py-3 py-1">
     
     <div className="grid md:grid-cols-3 grid-cols-2 gap-2">
       <div >
          <label className="font-semibold md:text-md text-sm">Blog Title</label>
        <input className="py-3 px-4 block w-full border border-gray-400 rounded-lg text-sm focus:border-blue-500 focus:ring-blue-500 disabled:opacity-50 disabled:pointer-events-none "   {...register("title", { required: true })} label="title" />
       </div>
       <div>
         <label className="font-semibold md:text-md text-sm">Blog Description</label>
         <textarea className="border py-3 px-4 block w-full border-gray-400 rounded-lg text-sm focus:border-blue-500 focus:ring-blue-500 disabled:opacity-50 disabled:pointer-events-none "  {...register("description")} label="description" />
        </div>
        <div>
        <label className="font-semibold md:text-md text-sm">image:</label>
      <input className="py-3 px-4 block w-full border-gray-400 rounded-lg text-sm focus:border-blue-500 focus:ring-blue-500 disabled:opacity-50 disabled:pointer-events-none "   {...register("image", { required: true })} type="file" accept="image/*" />
        </div>
     </div>
     ...
     <button className="md:ml-0 ml-4 md:px-2 px-1 rounded md:py-2 py-1 md:mt-4 my-2 md:w-[200px] w-[140px] hover:bg-white gap-2 border border-[#1EB2A6] hover:text-[#1EB2A6] bg-[#1EB2A6] text-white md:text-md text-sm flex justify-center items-center" 
      type="submit">{selectedBlogId ? "Update Blog" : "Create Blog"}</button>
    
   </form>
  <h2 className="text-gray-900 mx-auto md:w-[30%]  w-[70%] text-center my-6 border-x-2 border-[#1eb2a6] px-2 lg:text-2xl  capitalize md:text-xl text-lg font-semibold "> Blog Table</h2> 
